...
class Page():
	def __init__(self):
		...
		self.layout = column(row(self.select_data_files1,self.select_data_files2),
						self.refresh_button)
		self.tab1 = Panel(child = self.layout, title = "data")
		self.tab2 = Panel(child = self.select_data_files1, title = "plots")
		self.tabs = Tabs(tabs =[self.tab1,self.tab2,tab_invsible])
	...
	def update_page(self):
		...
		self.columns = [col for col in self.df1.columns if self.is_number(col)]
		
		self.select_plots = [0 for i in range(len(self.columns)*2)]
		...
		self.select_df1_corr = Select(title="Data frame 1:", value=self.columns[0], options=self.columns)
		self.select_df2_corr = Select(title="Data frame 2:", value=self.columns[1], options=self.columns)
		...
		self.layout.children = [row(self.select_data_files1,self.select_data_files2),
			self.plot_matrix,
		...
		self.tab1 = Panel(child = self.layout, title = "data")
		self.tab2 = Panel(child = self.layout, title = "data")
		#self.tabs.child = [self.tab1,self.tab2]
	...
	def update_directory(self):
		...
		self.select_data_files1 = Select(title="Data files 1:",value = "df1", options=["df1"]+self.data_files)
		self.select_data_files2 = Select(title="Data files 2:",value = "df2", options=["df2"]+self.data_files)
		self.layout.children[0] = row(self.select_data_files1,self.select_data_files2)
	...
	def init_plots_rock(self):
		...
		plots = column(row([self.draw_plot_rock(sdata1, col, yrange1, i == 0) for i, col in enumerate(self.columns)]),
			row([self.draw_plot_rock(sdata2, col, yrange2, i == 0) for i, col in enumerate(self.columns)]))
		return plots

	def init_corr(self):
		plot_correlation = figure(plot_width = 800, plot_height = 500, title = 'correlation graph')
		data = pd.DataFrame(data = {'x':self.df1[self.columns[0]],'y':self.df2[self.columns[1]]})
	...
	def update_plots(self,event):
		#self.r.glyph.text_font_size="1pt"
		length_columns = len(self.columns)
		x = int(event.x)#/self.plot_matrix.width)
		y = int(event.y) #/self.plot_matrix.height)
		if(self.select_plots[length_columns+x] == 1 and 
		...
		elif((event.x or event.x > length_columns < 0) and 
			(event.y < 0) or event.y > length_columns):
			return
		else:
			self.select_plots[length_columns+x] = 1
			self.select_plots[length_columns-y-1] = 1
		...
		plots1 = row([self.draw_plot_rock(sdata1, col, yrange1, i == 0) for i, col in enumerate(self.columns) if self.select_plots[i] == 1])
		plots2 = row([self.draw_plot_rock(sdata2, col2, yrange2, j == 0) for j, col2 in enumerate(self.columns) if self.select_plots[length_columns+j] == 1])
	...
	def update_predict(self):
		...
		plots1 = row([self.draw_plot_rock(sdata1, col, yrange1, i == 0) for i, col in enumerate(self.columns) if self.select_plots[i] == 1])
		plots2 = row([self.draw_plot_rock(sdata2, col2, yrange2, j == 0) for j, col2 in enumerate(self.columns) if self.select_plots[len(self.columns)+j] == 1])
